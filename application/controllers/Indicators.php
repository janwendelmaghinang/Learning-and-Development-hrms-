...
class Indicators extends Admin_Controller
{
	...
	{
		if (!in_array('viewIndicator', $this->permission)) {
			redirect('dashboard', 'refresh');
		}

		$this->data['departments'] = $this->model_departments->getDepartmentData();
		$this->data['key_indicator'] = $this->model_indicators->getKeyIndicatorData();
	...
	public function fetchIndicatorData()
	{
		...
		foreach ($data as $key => $value) {
			...
			$calculate = $this->model_indicators->calculateIndicator($value['id']);
			...
			$result['data'][$key] = array(
				...
				$department['name'],
				$designation['name'],
		        $calculate['rating'].' '.$calculate['indicator'],
				$status,
				$buttons
			);
		} // /foreach
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('leadership', 'Leadership', 'trim|required');
		$this->form_validation->set_rules('sales', 'Sales', 'trim|required');
		$this->form_validation->set_rules('enthusiasm', 'Enthusiasm', 'trim|required');
		$this->form_validation->set_rules('teamwork', 'Teamwork', 'trim|required');
		$this->form_validation->set_rules('integrity', 'Integrity', 'trim|required');
